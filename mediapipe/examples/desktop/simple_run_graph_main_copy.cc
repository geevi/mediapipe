...
::mediapipe::Status OutputStreamToLocalFile(
    ::mediapipe::OutputStreamPoller& poller, std::string output_stream_file) {
  std::ofstream file;
  file.open(output_stream_file);
  ...
  while (poller.Next(&packet)) {
    ...
    ::mediapipe::NormalizedLandmarkList nll = packet.Get<::mediapipe::NormalizedLandmarkList>();
    std::string pi = "";
    for(int i =0; i <nll.landmark_size(); i++) {
      pi = pi + std::to_string(nll.landmark(i).x()) + "," +  std::to_string(nll.landmark(i).y()) + ","; 
    }
    absl::StrAppend(&output_data, pi, "\n");
    file << output_data;
  }
  file.close();
...
::mediapipe::Status RunMPPGraph() {
  ...
  ASSIGN_OR_RETURN(auto poller,
                     graph.AddOutputStreamPoller("iris_landmarks"));
  ASSIGN_OR_RETURN(auto poller1,
                     graph.AddOutputStreamPoller("face_landmarks"));
  LOG(INFO) << "Start running the calculator graph.";
  MP_RETURN_IF_ERROR(graph.StartRun({}));
  MP_RETURN_IF_ERROR(OutputStreamToLocalFile(poller, absl::StrCat(FLAGS_output_stream_file,"_iris_landmarks.csv")));
  MP_RETURN_IF_ERROR(OutputStreamToLocalFile(poller1, absl::StrCat(FLAGS_output_stream_file,"_face_landmarks.csv")));
